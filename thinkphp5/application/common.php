...
function get_user_info($user_id_or_name,$type = 0,$oauth=''){
    ...
    $user = model('users')->where($map)->find();
    return $user;
}
...
function update_user_level($user_id){
    $level_info = model('user_level')->order('level_id')->select();
    $total_amount = model('order')->master()->where("user_id=:user_id AND pay_status=1 and order_status not in (3,5)")->bind(['user_id'=>$user_id])->sum('order_amount+user_money');
    if($level_info){
        ...
        model('users')->where("user_id", $user_id)->save($updata);
    }
}
...
function goods_thum_images($goods_id, $width, $height)
{
    ...
    $original_img = db('Goods')->cache(true, 3600)->where("goods_id", $goods_id)->getField('original_img');
    //dump($original_img);
...
function refresh_stock($goods_id){
    ...
    model("Goods")->where("goods_id", $goods_id)->save(array('store_count'=>$store_count)); // 更新商品的总库存
}
...
function minus_stock($order_id){
    $orderGoodsArr = model('OrderGoods')->master()->where("order_id", $order_id)->select();
    foreach($orderGoodsArr as $key => $val)
    {
        // 有选择规格的商品
        if(!empty($val['spec_key']))
        {   // 先到规格表里面扣除数量 再重新刷新一个 这件商品的总数量
            model('SpecGoodsPrice')->where(['goods_id'=>$val['goods_id'],'key'=>$val['spec_key']])->setDec('store_count',$val['goods_num']);
            refresh_stock($val['goods_id']);
        }else{
            model('Goods')->where("goods_id", $val['goods_id'])->setDec('store_count',$val['goods_num']); // 直接扣除商品总数量
        }
        model('Goods')->where("goods_id", $val['goods_id'])->setInc('sales_sum',$val['goods_num']); // 增加商品销售量
        //更新活动商品购买量
        if($val['prom_type']==1 || $val['prom_type']==2){
            $prom = get_goods_promotion($val['goods_id']);
            if($prom['is_end']==0){
                $tb = $val['prom_type']==1 ? 'flash_sale' : 'group_buy';
                model($tb)->where("id", $val['prom_id'])->setInc('buy_num',$val['goods_num']);
                model($tb)->where("id", $val['prom_id'])->setInc('order_num');
            }
        }
    }
}
...
function checkEnableSendSms($scene)
{
    ...
    //判断是否添加"注册模板"
    $size = model('sms_template')->where("send_scene", $scene)->count('tpl_id');
    if (!$size) {
...
function getCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['category_id_arr'] = model('GoodsCategory')->cache(true,TPSHOP_CACHE_TIME)->getField('id,parent_id');
    // 先把所有儿子找出来
    $son_id_arr = model('GoodsCategory')->where("parent_id", $cat_id)->cache(true,TPSHOP_CACHE_TIME)->getField('id',true);
...
function getArticleCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['cat_id_arr'] = model('ArticleCat')->getField('cat_id,parent_id');
    // 先把所有儿子找出来
    $son_id_arr = model('ArticleCat')->where("parent_id", $cat_id)->getField('cat_id',true);
    foreach($son_id_arr as $k => $v)
    {
...
function tpCache($config_key,$data = array()){
    ...
    if(empty($data)){
        ...
        if(empty($config)){
            //缓存文件不存在就读取数据库
            $res = model('config')->where("inc_type",$param[0])->select();
            if($res){
...
function get_goods_category_tree(){
    ...
    $cat_list = db('goods_category')->cache(true)->where("is_show = 1")->order('sort_order')->select();//所有分类
    if($cat_list){
