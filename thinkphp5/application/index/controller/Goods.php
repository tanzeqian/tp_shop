...
class Goods extends Base {
    ...
    */ 
    public function goodsInfo(){
        ...
        $goods_images_list = Db('GoodsImages')->where("goods_id", $goods_id)->select(); // 商品 图册
        $goods_attribute = Db('GoodsAttribute')->getField('attr_id,attr_name'); // 查询属性
        $goods_attr_list = Db('GoodsAttr')->where("goods_id", $goods_id)->select(); // 查询商品属性表
	    $filter_spec = $goodsLogic->get_spec($goods_id);
        //$freight_free = tpCache('shopping.freight_free'); // 全场满多少免运费
        $spec_goods_price  = Db('spec_goods_price')->where("goods_id", $goods_id)->getField("key,price,store_count"); // 规格 对应 价格 库存表
        Db('Goods')->where("goods_id", $goods_id)->update(array('click_count'=>$goods['click_count']+1 )); //统计点击数
